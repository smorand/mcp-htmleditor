...
| Module | Role |
|--------|------|
| `export/to_pptx.py` | HTML to PPTX. Slide detection (`data-type="slide"` on any tag, `article.slide` fallback), per charter chrome, vertical block flow, one renderer per component (text, grid, panel, table, gantt, arch diagram, annotated image, image, hbar). Gantt also has a class-free fallback, `_is_inline_gantt`: structural detection of a hand-authored, fully inline-styled Gantt (fixed-width label + `position:relative` track with absolutely positioned bars/markers), see `skill/types/gantt.md` § Variante. Returns an `ExportReport` (slide count, charter, warnings); the CLI prints it and exits non zero when nothing was exported. Traced as `export.pptx` |
