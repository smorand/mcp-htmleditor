...
- [EXC-007b] (résolu, DEC-014) Port déjà occupé par un autre process → si aucun port n'est demandé explicitement, auto-sélection d'un port libre (7842 en priorité, sinon 7840-7849, voir FR-009 et FR-029) au lieu d'un crash `OSError` brut; si un port est demandé explicitement et occupé, erreur claire (`OSError`/`click.ClickException`) plutôt qu'une trace Python brute.
...
- **Business Rules:** `get_status` ne doit jamais lever d'exception, même sans fichier chargé. `start_server` sans `port` explicite délègue à FR-029 (auto-sélection de port) plutôt que de fixer 7842 en dur.
- **Priority:** Must-have

### FR-029 [EARS-O]: Auto-sélection de port pour la coexistence de plusieurs présentations
> IF aucun port n'est demandé explicitement (CLI `--port` omis, ou outil MCP `start_server` appelé sans `port`) THEN THE système SHALL essayer le port préféré (`HTMLEDITOR_PORT`, 7842 par défaut) puis, s'il est occupé, scanner 7840-7849 dans l'ordre et se lier au premier port libre trouvé.

- **Inputs:** État d'occupation des ports 7840-7849 au moment du démarrage (`find_free_port()`).
- **Outputs:** Port réellement lié, remonté dans la réponse de l'outil MCP (`port`), dans le message CLI (`Serving on http://...`), et dans `EditorState.port`/`.mcp_state.json`.
- **Business Rules:** Un port explicitement demandé (`--port N` ou `start_server(file, port=N)`) est utilisé tel quel, sans repli automatique: s'il est occupé, erreur claire (`OSError` côté MCP, `click.ClickException` côté CLI) plutôt qu'un silence ou une trace Python brute. Si les 10 ports (7840-7849) sont tous occupés, `RuntimeError` explicite ("10 présentations déjà en cours ?"). Chaque `mcp-htmleditor serve`/`mcp-htmleditor mcp` est un process indépendant (état singleton par process, voir FR-010): l'auto-sélection permet donc à 10 processus indépendants de cohabiter sans collision, mais **pas** à une seule session agent de piloter plusieurs serveurs à la fois (limite architecturale inchangée, cf. TBD-002 et le choix explicite de l'utilisateur de rester sur ce modèle plutôt que de refondre l'état en registre multi-instances).
- **Priority:** Must-have
- **Rationale:** Avant ce fix, deux décks ouverts sans `--port` explicite entraient en collision sur 7842 (un `OSError` non géré faisait crasher le second serveur) — EXC-007b, gap identifié lors de la rétrospective initiale.
...
| (transverse) | FR-029 | E2E-042 | E2E-044 | E2E-043 |
...
| E2E-042 | Existant (`test_port_range.py::test_returns_the_preferred_port_when_free`, `test_two_presentations_get_two_different_ports`) | Happy | (transverse) | FR-029 | Critical |
| E2E-043 | Existant (`test_port_range.py::test_falls_back_into_the_range_when_preferred_is_taken`) | Edge | (transverse) | FR-029 | High |
| E2E-044 | Existant (`test_port_range.py::test_raises_a_clear_error_when_the_whole_range_is_taken`) | Failure | (transverse) | FR-029 | Medium |
...
**Coverage Statistics:**
- Happy path: 13 tests
- Failure/error: 16 tests
- Edge cases: 15 tests
- Happy:Failure ratio: 1:1.23 (failure ≥ happy, conforme à la règle)
- Tests existants réutilisés: 34 (déjà présents dans `tests/*.py`, dont `test_port_range.py` ajouté avec FR-029)
...
- **DEC-013:** Fix du flash au changement de slide en plein écran (la slide apparaissait d'abord à sa taille normale, puis "sautait" brusquement au zoom). Cause: l'animation d'entrée `@keyframes fadein` (200ms, existante avant le mode plein écran) animait la propriété `transform` (`translateY`), qui écrasait temporairement `:fullscreen .slide { transform: scale(var(--fs-scale)) }` (DEC-012) pendant toute la durée de l'animation — une fois celle-ci terminée, le scale reprenait la main d'un coup, visible à l'œil nu. **Fix (option choisie par l'utilisateur parmi 3 proposées):** séparer `scale` et `translate` en propriétés CSS indépendantes (supportées nativement, non liées à `transform`) plutôt que de tout empiler dans `transform`: `@keyframes fadein` anime desormais `translate:0 6px → 0 0`, et `:fullscreen .slide` utilise `scale: var(--fs-scale, 1)` au lieu de `transform: scale(...)`. Les deux propriétés se composent sans jamais s'écraser mutuellement, plus de fenêtre où l'une masque l'autre. **Alternatives proposées et écartées:** désactiver l'animation en plein écran (simple mais perd l'effet de fondu/glissement), dupliquer un jeu de `@keyframes` spécifique au plein écran (fonctionne mais duplique du CSS par template). **Validation:** échantillonnage Playwright de `getBoundingClientRect()` à 20/60/100/150/250ms après changement de slide — taille visuelle `[1600, 900]` stable dès 20ms, aucune régression vers la taille non zoomée pendant les 200ms de l'animation. **Propagation:** mêmes 3 fichiers que DEC-012 (`euro-information.html` source + `slides-ei-empty.html` régénéré, `slides-empty.html` Carbon, `Agentic_Platform_Deck.html` de l'utilisateur). **Round:** rétrospectif, post-publication.
- **DEC-014:** Auto-sélection de port (7842 en priorité, sinon 7840-7849) pour laisser plusieurs présentations cohabiter, à la demande de l'utilisateur. **Décision de portée:** l'utilisateur a confirmé vouloir uniquement la coexistence de *process indépendants* (plusieurs `mcp-htmleditor serve`/sessions agent, chacun avec son propre port), pas un registre multi-instances dans un seul process — l'architecture singleton existante (`EditorState`, `_server_instance` process-wide) reste inchangée, ce qui garde le changement petit et sans risque. **Fix:** `find_free_port(host, preferred)` (nouveau, `http_server.py`) essaie le port préféré puis scanne la plage; `start_http_server` accepte `port: int | None` (None = auto, explicite = respecté tel quel avec erreur claire si occupé au lieu d'un `OSError` brut non géré); CLI (`serve`, `new --serve`) et outil MCP `start_server` ne résolvent plus le port par défaut avant l'appel, laissent l'auto-sélection opérer, et remontent le port réellement utilisé (jamais supposer 7842 en dur). **Alternatives considérées:** transformer l'état en registre multi-instances pour permettre à un seul agent de piloter plusieurs serveurs à la fois (écartée par l'utilisateur: gros changement d'architecture, mériterait sa propre interview de spec plutôt qu'un patch rapide — voir la question posée et la réponse "Plusieurs process indépendants"). **Validation:** `tests/test_port_range.py` (4 tests: port préféré libre, repli dans la plage, deux présentations simultanées sans collision, erreur claire si les 10 slots sont pris) + test manuel de deux vrais process `mcp-htmleditor serve` coexistant sur 7840/7842. **Documentation:** `README.md` (section "Multiple presentations at once"), `skill/SKILL.md` (section "Plusieurs présentations en parallèle"), `skill/workflow-create.md` (ne plus supposer 7842 en dur pour la validation visuelle). **Round:** rétrospectif, post-publication (demande explicite de l'utilisateur).
- **DEC-015:** Alignement du canevas Carbon sur celui d'EI (960×540, ratio 16:9 fixe), à la demande explicite de l'utilisateur. **Contexte:** en plein écran, une slide Carbon avec peu de contenu zoomait vers une boîte disproportionnée (900×294 mesuré → 1600×519 zoomé, grandes bandes noires) alors qu'EI remplissait l'écran en 16:9 propre — cause racine: Carbon n'avait **jamais** eu de canevas fixe (`.slide { max-width:900px; overflow-y:auto; max-height:100%; }`, hauteur pilotée par le contenu, scroll en repli), contrairement à EI (`aspect-ratio:16/9` dès l'origine). **3 options proposées à l'utilisateur:** (a) canevas fixe uniquement en plein écran (chirurgical, ne touche pas le mode édition), (b) canevas fixe partout y compris édition (aligne les deux chartes en profondeur, mais change le comportement de tous les decks Carbon existants), (c) ne rien changer, documenter la limite. **L'utilisateur a choisi (b).** **Fix:** `.slide` passe de `max-width:900px; overflow-y:auto; max-height:100%;` à `max-width:960px; aspect-ratio:16/9; overflow:hidden;` (identique à EI au pixel). Les enfants (`.slide-body { flex:1; }`, `.slide-footer { flex-shrink:0; }`) étaient déjà conçus en flex pour un parent à hauteur fixe (la classe `.slide.dense` existait déjà pour gérer un budget de hauteur, jamais réellement imposé avant ce fix) — aucun changement nécessaire côté enfants. **Effet de bord accepté par l'utilisateur:** le contenu qui dépassait 540px et scrollait avant est désormais rogné en silence (`overflow:hidden`), comme cela l'était déjà pour EI. **Validation:** mesure Playwright (slide windowed = [960, 540] ratio 16:9 exact, `overflow-y:hidden`; plein écran = `--fs-scale` = 1600/960 exact, visuel = [1600, 900], toolbar/shell-header masqués) — parité totale avec EI confirmée. **Documentation:** `skill/types/slides.md` section "Hauteur utile d'une slide" réécrite (plus de formule dépendante du viewport, plafond dur 540px au lieu d'un seuil de scroll souple à 576px), règle 8/9 de "Règles de construction" mises à jour, avertissement sur `ibm-carbon.html` renforcé (voir TBD-011). **Round:** rétrospectif, post-publication (demande explicite de l'utilisateur, question posée avant implémentation).
